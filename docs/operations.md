...
2. 双击 `启动AI创意工作台.bat`。
3. 浏览器打开 `http://127.0.0.1:8775/`。
4. 先访问 `http://127.0.0.1:8775/api/health`，确认返回成功。
5. 需要验证 AI 网关时，再访问 `http://127.0.0.1:8780/v1/models`。

## 停止

关闭启动窗口会停止网页服务；网关窗口由启动器单独拉起。更新代码或配置前，应先确认 `8775` 和 `8780` 已停止，再进行备份和替换。
