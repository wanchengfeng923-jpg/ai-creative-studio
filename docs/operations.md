...
当前已临时开放同一局域网访问：网页服务监听 `0.0.0.0:8775`，并由 Windows 防火墙仅允许 `LocalSubnet`；AI 网关仍监听 `127.0.0.1:8780`。网页已有登录和项目归属控制，但仍不适合公网暴露。
...
4. 点击“启动”；控制台会拉起本项目自己的网关和网页，并自动打开 `http://192.168.1.135:8775/`。
5. 可在控制台点击“检测连接”，或访问 `http://192.168.1.135:8775/api/health` 和 `http://127.0.0.1:8780/v1/models`。
