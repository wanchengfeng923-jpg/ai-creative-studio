...
首次刷新时会显示“正在刷新状态：端口、进程和防火墙...”，完成后显示“状态已刷新”。刷新不会把整个面板锁成灰色；上线成功时显示的 `公网 Web 进程已启动，PID xxxx。` 是进程标识，不是错误。
...
“可执行文件不匹配”“工作目录不匹配”“入口命令不匹配”表示面板暂时无法确认该 PID 属于本项目。面板允许不同 Python 版本，也支持启动器与服务之间存在多级父进程；但没有项目归属证据时不会结束进程或上线公网。不要直接结束所有 Python，也不要同时手动启动第二套网关或 Web。
